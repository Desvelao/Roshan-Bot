...
const loader = new AssetLoader(path.resolve(__dirname, '../img/'), {
  hero: {
    path: 'heroes',
    ext: 'png',
    conversor: (input) => enumHeroes.getValue(input).name_id
  },
  minihero: {
    path: 'miniheroes',
    ext: 'png',
  ...
  item: {
    ...
    ext: 'png',
    conversor: (input) => {
      const result = enumItems.getValue(input);
      return result ? result.name_id : undefined;
    }
  },
  cardBG: {
    path: 'cards',
    ext: 'jpg',
    conversor: (input) => enumCardBG.getValue(input)
  },
  ...
  medal: {
    path: 'medals',
    ext: 'png',
    conversor: (input) => enumMedals(input).compose
  }
});
...
loader.fontLoad = function (font) {
  return this.font(this._enum.fonts[font]);
};

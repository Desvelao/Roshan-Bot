...
module.exports = function (data_info) {
  ...
  jimps.push(
    Promise.all([
      jimp.read(player.profile.avatarmedium),
  ...
  return Promise.all(jimps).then((data) => {
    ...
    const player_avatar = canvas.paint(
      'player_avatar',
      data[2][0].resize(24, jimp.AUTO)
    );
    const player_name = canvas
      .write('player_name', data[2][1], 'w16')
      .place(player_avatar, 'rxcy', { x: 10 });
    const player_group = canvas
      .group('player_group', [player_avatar, player_name])
      .set(canvas, 'cxty', { y: 10 });
    for (var i = MATCHES_START_INDEX, l = jimps.length; i < l; i++) {
      const y =
        (i - MATCHES_START_INDEX) * (HERO_HEIGHT + 2) + POSITION.TABLE.Y;
      const match = data[i];
      const heroimg = canvas
        .paint('heroimg', match[0].resize(jimp.AUTO, 24), {
          x: POSITION.TABLE.X,
          y
        })
        .add();
      const victory = canvas
        .write('victory', match[1], 'w16')
        .set(heroimg, 'rxcy', { x: 10 });
      const kda = canvas
        .write('kda', match[2], 'w16', { x: 130 })
        .place(null, 'csx')
        .set(heroimg, 'cy');
      const duration = canvas
        .write('duration', match[3], 'w16', { x: 200 })
        .place(null, 'csx')
        .set(heroimg, 'cy');
      const match_id = canvas
        .write('match_id', match[4], 'w16', { x: duration.x + duration.w + 10 })

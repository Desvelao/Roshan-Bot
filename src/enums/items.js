...
enumItems.getValueByName = function (tag) {
  ...
  for (const [key, val] of this) {
    if (
      typeof val.dname !== 'string' ||
      val.dname.toLowerCase().includes('recipe')
    ) {
      continue;
    }
    if (val.dname.toLowerCase().includes(tag.toLowerCase())) {
      results.push(val);
    }
  }
  if (results.length) {
    const exact = results.find(
      (result) => result.dname.toLowerCase() === tag.toLowerCase()
    );

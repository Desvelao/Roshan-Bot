...
module.exports = new Command(
  'duel',
  {
    category: 'Fun',
    hide: true,
    check: (msg, args, client, command) =>
      msg.channel.id === client.components.DuelGame.channelMatchmakingID
  },
      ...
      buttons: [
        {
          ...
          add: (msg, emoji, userID, client, that) => {
            if (
              userID !== that.author.id &&
              client.server.members.has(that.author.id) &&
              client.server.members.has(userID)
            ) {
              const player1 = {
                id: that.author.id,
                name: that.author.username,
                avatar: that.author.avatarURL,
                rank: 0,
                color: 1000000,
                mention: `<@${that.author.id}>`
              };
              ...
              return that
                ...
                .then(() =>
                  client.createChannel(
                    client.server.id,
                    `Lobby ${player1.name} vs ${player2.name}`,
                    0,
                    'Duel game',
                    { parentID: client.components.DuelGame.categoryID }
                  )
                )
                .then((channel) => {
                  // Create channel lobby and give permissions to players
                  return Promise.all([
                    channel.editPermission(player1.id, 1024, null, 'member'),
                    channel.editPermission(player2.id, 1024, null, 'member')
                  ]).then(() => {
                    const board = new Board({
                      ...
                      buttons: gameconfig.board.buttons.map((button) => ({
                        button: button
                      }))
                    });
                    board.locale = (string, extra) =>
                      client.components.DuelGame.findAndReplace(string, extra);
                    board.replaceContent = (content, extra) =>
                      client.components.DuelGame.replaceContent(content, extra);
                    board.createMessage = function (content, extra, file) {
                      return client.components.DuelGame.createMessage(
                        this.channel,
                        content,
                        extra,
                        file

...
module.exports = class DuelGame extends Component {
  constructor(client, options) {
    ...
    this.locale = {};
    this.client.on(
      'duel:awaitmanager:messageCreate',
      (err, msg, client, message) => {
        console.log('Error: messageCreate', message.id, err);
      }
    );
    this.client.on(
      'duel:awaitmanager:messageReactionAdd',
      (err, msg, emoji, userID, client, message) => {
        console.log('Error: messageReactionAdd', message.id, err);
      }
    );
    this.client.on(
      'duel:awaitmanager:messageReactionRemove',
      (err, msg, emoji, userID, client, message) => {
    ...
    this.client.on('duel:closegame', (board) => {
      this.client.createMessage(this.channelLogID, {
        embed: board.resultMessage
      });
    });
  }
  ready() {
    Object.keys(gameconfig.emojis).forEach((emojiKey) =>
      this.addLocale('emoji_' + emojiKey, gameconfig.emojis[emojiKey])
    );
  ...
  testBoard() {
    ...
    board.locale = (string, extra) =>
      self.client.components.DuelGame.findAndReplace(string, extra);
    board.replaceContent = (content, extra) =>
      self.client.components.DuelGame.replaceContent(content, extra);
    board.createMessage = function (content, extra, file) {
      return self.client.components.DuelGame.createMessage(
        this.channel,
        content,
        extra,
        file

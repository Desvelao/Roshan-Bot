...
module.exports = class DashboardMessage extends AwaitMessage {
  ...
  init() {
    return super
      ...
      .then(() =>
        this.buttons.reduce(
          (promise, action) => promise.then(() => this.addActionButton(action)),
          Promise.resolve()
        )
      );
  }
  addActionButton(action) {
    return this.message
      .addReaction(action.button)
      .then(() => this.registerAction(action));
  }
  removeActionButton(action) {
    return this.message
      .removeReaction(action.button)
      .then(() => this.unregisterAction(action));
  }
  destroyMessage() {
    this.unregister();
  ...
  messageCreate(msg, client) {
    if (this.message && msg.channel.id === this.message.channel.id) {
      const command = this.commands.find((command) =>
        msg.content.startsWith(command.name)
      );
      if (command) {
  ...
  messageReactionRemove(msg, emoji, userID, client) {
    if (
      ...
      if (this.actions['remove'][emoji.name]) {
        return this.actions['remove'][emoji.name](
          msg,
          emoji,
          userID,
          client,

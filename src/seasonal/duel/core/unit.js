module.exports = class BasicUnit {
  constructor(stats = {}, owner) {
    ...
    this.owner = owner;
    this.alive = true;
    if (stats.onAddPlayer) {
      this.onAddPlayer = stats.onAddPlayer;
    }
    if (stats.onRemovePlayer) {
      this.onRemovePlayer = stats.onRemovePlayer;
    }
  }
  get isHero() {
  ...
  get isDisarmed() {
    return (
      this.modifiers.some((modifier) => modifier.disarm) ||
      this.owner.modifiers.some((modifier) => modifier.disarm)
    );
  }
  ...
  statBonus(s) {
    return (
      this.modifiers
        .filter(
          (modifier) =>
            modifier.affect && modifier.affect.includes('self') && modifier[s]
        )
        .reduce((sum, modifier) => {
          sum += modifier[s] || 0;
          return sum;
        }, 0) +
      this.owner.modifiers
        .filter(
          (modifier) =>
            modifier.affect &&
            modifier.affect.includes(this.type) &&
            modifier[s]
        )
        .reduce((sum, modifier) => {
          sum += modifier[s] || 0;
  ...
  get expiration() {
    return typeof this._expiration === 'number'
      ? this.stat('_expiration')
      : this._expiration;
  }
  render() {
    return `${this.name} (${this.type}) ${this.atk}/${this.rst}/${this.hp}${
      this.isDisarmed ? ' <emoji_disarm>' : ''
    }`;
  }
  healing(value) {
    this._hp += value;

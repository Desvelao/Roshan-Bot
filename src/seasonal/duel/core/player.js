...
class Player {
  ...
  get enemy() {
    return this.board.players.find(
      (player) => player.playerIndex === (this.playerIndex === 1 ? 0 : 1)
    );
  }
  emit(event, ...args) {
  ...
  get hp() {
    return (
      ...
      this.modifiers
        .filter(
          (modifier) =>
            modifier.affect && modifier.affect.includes('player') && modifier.hp
        )
        .reduce((sum, modifier) => sum + modifier.hp, 0) +
      this.units
        .map((unit) => unit.modifiers)
        .filter(
          (modifier) =>
            modifier.affect && modifier.affect.includes('player') && modifier.hp
        )
    ...
    return (
      ...
      this.modifiers
        .filter(
          (modifier) =>
            modifier.affect &&
            modifier.affect.includes('player') &&
            modifier.mana
        )
        .reduce((sum, modifier) => sum + modifier.mana, 0) +
      this.units
        .map((unit) => unit.modifiers)
        .filter(
          (modifier) =>
  ...
  get gold() {
    return (
      ...
      this.modifiers
        ...
        .reduce((sum, modifier) => sum + modifier.gold, 0) +
      this.units
        .map((unit) => unit.modifiers)
        .filter(
          (modifier) =>
  ...
  destroyedMana(value, source) {
    ...
    this.createNotification(
      `destroyed **${value}** <emoji_mana> by **${source.name}**`
    );
  }
  producedMana(value, source) {
    this.addMana(value);
    this.createNotification(
      `produced **${value}** <emoji_mana> by **${source.name}**`
    );
  }
  destroyedGold(value, source) {
    this.consumeGold(value);
    this.createNotification(
      `destroyed **${value}** <emoji_gold> by **${source.name}**`
    );
  }
  producedGold(value, source) {
    this.addGold(value);
    this.createNotification(
      `produced **${value}** <emoji_gold> by **${source.name}**`
    );
  }
  addModifier(modifier) {
    this.modifiers.push(modifier);
    this.createNotification(
      `modifier added **${modifier.source}** - ${modifier.description}`
    );
  }
  removeModifier(modifier) {
    this.modifiers = this.modifiers.filter((m) => m !== modifier);
    this.createNotification(
      `modifier removed **${modifier.source}** - ${modifier.description}`
    );
  }
  attack(targetPlayer) {
  ...
  get passTurn() {
    ...
    return {
      ...
      play() {
        self.board.addHistoryAction({
          type: 'pass',
          description: `**${self.name}** pass`,
          source: self
        });
      }
    };
  ...
  removeUnit(unit) {
    if (unit === this.hero) {
      ...
      this.addModifier({
        ...
        source: unit.name,
        description: `Revive hero ${unit.name} with hp base. When round ${
          this.board.round + expiration
        } starts`,
  ...
  render() {
    return {
      name: `${this.name}: ${this.mana} <emoji_mana> ${
        this.gold
      } <emoji_gold> ${this.atk}/${this.rst}/${this.hp} ${
        this.checkLetal(this.enemy) ? '<emoji_skull>' : ''
      }${this.isSilenced ? ' <emoji_silence>' : ''}`,
      value: this.units.map((unit) => unit.render()).join('\n') || 'No army',
      inline: true
    };
  }
  acquireSkill(skill) {
  ...
  emit(event) {
    this.modifiers
      .filter((modifier) => modifier.event && modifier.event === event)
      .forEach((modifier) => {
        if (modifier.run) {

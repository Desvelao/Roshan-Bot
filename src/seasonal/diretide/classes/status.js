...
module.exports = class StatusFirebase {
  ...
  statusTo(mode, doing = 0) {
    mode = parseInt(mode);
    return this.save({ mode }).then((status) =>
      Promise.all([
        this.game.client.plugins.Bot.setStatus(
          doing,
          statusBot[status.mode],
          this.statusMsg(status.mode)
        ),
        this.game.log(
          this.embed({
            description: `Roshan status changed to ${enumStatus[status.mode]}`
          })
        )
      ])
    );
  ...
  statusToActive() {
    return this.statusTo(0);
  }
  // statusToExhausted(){return this.statusTo(1)}
  statusToSugarRush() {
    return this.statusTo(1).then(() => {
      ...
      this.timeouts.active = new Timeout(() => {
        this.statusToActive();
        if (this.chainSugarRush) {
          this.randomSugarRush(this.game.config.events.sugarrush.randomStart);
        }
      }, delay);
  ...
  randomSugarRush(intervalTime) {
    const time =
      this.game.actions.intervalRandomNumber(
        intervalTime.map((v) => v * 3600)
      ) * 1000; // TODO change 2 by 3600 to conver seconds interval in hours
  ...
  stopcycle() {
    this.clearTimers();
    return this.statusTo(0).then(() =>
      this.game.log(this.embed({ description: ':x: Event cycle stopped' }))
    );
  }
  startcycle() {
    this.clearTimers();
    this.statusTo(0).then(() =>
      this.randomSugarRush(this.game.config.events.sugarrush.randomStart)
    );
  }
  getModeName() {
...
function minutesToS(time) {
  return time * 60;
}
function minutesToMS(time) {
  return time * 60 * 1000;
}
function hoursToMS(time) {

...
module.exports.getCacheUserMentioned = function (msg) {
  return msg.mentions && msg.mentions.length
    ? this.actions.getUser(msg.mentions[0].id)
    : false;
};
...
module.exports.reqUserShow = function (requirements) {
  return (
    ...
    Object.keys(requirements)
      .map((key) => `${this.config.emojis[key]}: ${requirements[key]}`)
      .join(', ')
  );
};
...
module.exports.rewardsUser = function (rewards) {
  return keys(rewards).reduce((gift, key) => {
    ...
    if (isNumber(reward)) {
      ...
      if (isUndefined(reward.notsecure)) {
        gift[key] = this.actions.rewardsUser(rewards[key]);
        return gift;
      }
      ...
      if (isNumber(reward.notsecure)) {
        if (reward.secure) {
          gift[key] = reward.secure;
        }
        if (reward.chance < 1 && this.actions.chanceSuccess(reward.chance)) {
          gift[key] = (gift[key] || 0) + reward.notsecure;
        }
      } else if (isBoolean(reward.notsecure)) {
        if (reward.chance < 1 && this.actions.chanceSuccess(reward.chance)) {
          gift[key] = reward.notsecure;
        }
        ...
        if (reward.chance < 1 && this.actions.chanceSuccess(reward.chance)) {
          gift[key] =
            (gift[key] || 0) +
            this.actions.intervalRandomNumber(reward.notsecure);
        }
      }
    }
    return gift;
...
module.exports.totalUserReqRew = function (cached, req, rew) {
  ...
  return keys(user).reduce((user, key) => {
    if (!isObject(user[key])) {
      ...
      user[key] = this.actions.totalUserReqRew(
        user[key],
        req[key] || {},
        rew[key] || {}
      );
...
module.exports.calcReqRew = function (user, req, rew) {
  ...
  const stats = (result.total = this.actions.totalUserReqRew(
    user,
    result.requirements,
    result.rewards
  ));
...
module.exports.showRewards = function (rewards) {
  return Object.keys(rewards)
    .sort()
    .filter((key) => ['candies', 'essence', 'special'].includes(key))
    .map((key) => `${this.config.emojis[key]}: ${rewards[key]}`)
    .join(', ');
};
...
module.exports.newUser = function (id) {
  ...
  const team = Object.keys(teams)
    .map((key) => ({ _id: key, size: teams[key] }))
    .sort((a, b) => a.size - b.size)[0]._id;
...
module.exports.resetTeam = function (id) {
  if (Object.keys(!this.config.teams).includes(id)) {
    return;
  }
  const initialTeam = {
    candies: 100,
...
module.exports.retryCommand = function (bot, msg, args, command) {
  return this.actions
    .newUser(msg.author.id || msg)
    .then(() => command.process.call(bot, msg, args, command));
};

module.exports.retryCommandWithError = function (bot, msg, args, command) {
  return this.actions
    .retryCommand(bot, msg, args, command)
    .then(() => command.error());
};

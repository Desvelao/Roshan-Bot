      ...
      game.status.embed({
        ...
        fields: teams.map((team) =>
          teamInfo(
            ...
            game.actions.nowToTimestamp,
            {
              candies: game.config.emojis.candies,
              protect: game.config.emojis.protect
            }
          )
        ),
...
function teamInfo(team, toReadTime, now, emojis) {
  return {
    name: team._id,
    value: `${emojis.candies}: ${team.candies}\n${emojis.protect}: ${toReadTime(
      team.ts.steal - now()
    )}`,
    inline: true
  };

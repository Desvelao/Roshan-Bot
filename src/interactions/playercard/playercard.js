...
module.exports = {
  ...
  run: async function (interaction, client, command) {
    ...
    if (account.card.heroes.split('').length < 1) {
      return client.components.Opendota.card_heroes(account.dota)
        .then((results) => {
          account.card.heroes = results[1]
            .slice(0, 3)
            .map((h) => h.hero_id)
            .join(',');
        ...
        .then((buffer) =>
          client.components.Locale.replyInteraction(interaction, {
            file: {
              file: buffer,
              name: interaction.user.username + '_roshan_card.png'
            }
          })
        )
        .catch((err) =>

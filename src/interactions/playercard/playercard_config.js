...
module.exports = {
  ...
  run: async function (interaction, client, command) {
    const heroesOption = interaction.data.options.find(
      (option) => option.name === 'heroes'
    );
    const positionOption = interaction.data.options.find(
      (option) => option.name === 'position'
    );
    const backgroundOption = interaction.data.options.find(
      (option) => option.name === 'background'
    );
    ...
    if (positionOption) {
      ...
      if (!position) {
        return client.components.Locale.replyInteraction(
          interaction,
          'playercard.error.req3heroes'
        ); // TODO: error position configuration
      }
    ...
    return client.components.Locale.replyInteraction(
      interaction,
      {
        embed: {
          ...
          fields: [
            {
              name: 'playercard.highlightsheroes',
              value: '{{{_heroes}}}',
              inline: false
            },
      ...
      },
      {
        ...
        _heroes: `\`\`\`${
          interaction.user.account.card.heroes
            ? interaction.user.account.card.heroes
                .split(',')
                .map((h) => enumHeroes.getValue(h).localized_name)
                .join(', ')
            : ''
        }\`\`\``,
        _position: `\`\`\`${enumPlayerPosition.getValue(
          interaction.user.account.card.pos
        )}\`\`\``,
        _bg: `\`\`\`${enumPlayerCardBg.getValue(
          interaction.user.account.card.bg
        )}\`\`\``
      }
    );
  }

...
module.exports = {
  ...
  run: async function (interaction, client, command) {
    const hero = enumHeroes.getValueByAlias(
      interaction.data.options.find((option) => option.name === 'hero').value
    );
    if (!hero) {
    ...
    const heroBaseHealth =
      hero.base_health +
      Math.round((hero.primary_attr === 'str' ? 22.5 : 17) * hero.base_str);
    const heroBaseMana =
      hero.base_mana + (hero.primary_attr === 'int' ? 15 : 12) * hero.base_int;
    const heroBaseArmor =
      hero.base_armor +
      Math.round((hero.primary_attr === 'agi' ? 0.2 : 0.16) * hero.base_agi);
    ...
    return client.components.Locale.replyInteraction(interaction, {
      embed: {
        author: {
          ...
          icon_url: `${enumHeroes.dotaCdnURL}${hero.icon}`,
          url: `${enumHeroes.dotaWikiURL}${hero.localized_name}`.replace(
            ' ',
            '%20'
          )
        },
        ...
        fields: [
          {
            name: 'Stats',
            value: `Base health: ${heroBaseHealth}\nBase mana: ${heroBaseMana}\nBase armor: ${heroBaseArmor}\nAttack type: ${heroAttackType}\nRange: ${heroRange}\nMS: ${heroMovementSpeed}\n`,
            inline: true
          },
          {
            name: 'Attributes',
            value: `${highlightAttributeFromHero('str', hero, 'Str')}: ${
              hero.base_str
            } + ${hero.str_gain}\n${highlightAttributeFromHero(
              'agi',
              hero,
              'Agi'
            )}: ${hero.base_agi} + ${
              hero.agi_gain
            }\n${highlightAttributeFromHero('int', hero, 'Int')}: ${
              hero.base_int
            } + ${hero.int_gain}`,
            inline: true
          },

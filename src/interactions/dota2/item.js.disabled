...
module.exports = {
  ...
  run: async function (interaction, client, command) {
    const item = enumItems.getValueByName(
      interaction.data.options.find((option) => option.name === 'item').value
    );
    ...
    return client.components.Locale.replyInteraction(interaction, {
      embed: {
        author: {
          name: item.dname,
          url: enumItems.dotaWikiURL + item.dname.replace(/ /g, '_')
        },
        ...
        fields: [
            ...
            ? [
                {
                  ...
                  value: item.attrib
                    .map(
                      (attr) =>
                        `*${attr.header}* ${attr.value} ${attr.footer || ''}`
                    )
                    .join('\n'),
                  inlue: true
                }
          ...
          ...(item.components && item.components.length
            ? [
                {
                  name: `Components - Total cost: ${item.cost}`,
                  value: item.components
                    .map(
                      (component) => `**${component.dname}** ${component.cost}`
                    )
                    .join(', '),
                  inlue: true
                }
            ...
            : []),
          ...(item.active && item.active.length
            ? [{ name: 'Actives', value: '<_item_actives>', inlue: true }]
            : []),
          ...(typeof item.cd === 'number'
            ? [{ name: 'Cooldown', value: item.cd, inlue: true }]
            : [])
        ],

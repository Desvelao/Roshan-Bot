...
module.exports = {
  ...
  run: async function (interaction, client, command) {
    ...
    const configuration = interaction.data.options.find(
      (option) => option.name === 'configuration'
    ).value;
    ...
    const limit = 4;
    if (
      [TOURNAMENT_MODE.ELIMINATION, TOURNAMENT_MODE.GROUPS].indexOf(mode) ==
        -1 &&
      list.length < limit
    ) {
      return client.components.Locale.replyInteraction(
        interaction,
        'tournament.error.limitelements',
        { limit }
      );
    }
    if (mode == TOURNAMENT_MODE.ELIMINATION) {
      ...
      for (var i = 0; i < result.length; i += 2) {
        if (result[i + 1]) {
          ...
          fields[0].value += '**' + result[i] + '** :arrow_forward: \n';
        }
      }
    } else if (mode == TOURNAMENT_MODE.GROUPS) {
      var numberGroups = parseInt(args[2]);
      if (typeof numberGroups !== 'number' || isNaN(numberGroups)) {
        return client.components.Locale.replyInteraction(
          interaction,
          'tournament.error.groupsnum'
        );
      }
      const list = args.slice(3).join('').split(',');
      if (list.length < numberGroups * 2) {
        return client.components.Locale.replyInteraction(
          interaction,
          'tournament.error.groupsnumhigh'
        );
      }
      var groups = [];
      ...
      for (var i = list.length - 1; i > -1; i--) {
        ...
        list.splice(random, 1);
        group++;
        if (group > numberGroups - 1) {
          group = 0;
          element = groups[0].length;
        }
      }

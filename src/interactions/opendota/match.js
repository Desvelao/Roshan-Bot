  ...
  run: async function(interaction, client, command){
    return client.components.Opendota.match(interaction.data.options.find(option => option.name === 'match_id').value)
      .then(results => {
        ...
        let radiant = new util.Classes.Table(headers, null, spacesBoard, { fill: '\u2002' })
        let dire = new util.Classes.Table(headers, null, spacesBoard, { fill: '\u2002' })
        ...
        results[0].players.forEach((player, index) => {
          if (index < 5) {
            radiant.addRow([
              ...
              util.Number.tok(player.hero_damage) + client.components.Locale._replaceContent('number.k', interaction.user.account.lang),
              util.Number.tok(player.tower_damage) + client.components.Locale._replaceContent('number.k', interaction.user.account.lang),
        ...
        return client.components.Locale.replyInteraction(interaction, {
          embed: {
            ...
            fields: [
              {name : '{{{match_field0_name}}}', value: '{{{match_field0_value}}}', inline: false},
              {name : '{{{match_field1_name}}}', value: '{{{match_field1_value}}}', inline: false},
            ]
          }
        },{
          ...
          match_field0_name: (results[0].radiant_team ? results[0].radiant_team.name : client.components.Locale._replaceContent('dota2.radiant', interaction.user.account.lang)) + ' - ' + results[0].radiant_score,

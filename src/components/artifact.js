...
class Artifact extends Component {
  constructor(client, options) {
    super(client);
    this.enable = this.client.isProduction;
    this.cards = [];
    this.sets = [];
    this.cardsURL =
    ...
    this.suffixSig = ':sig';
    this.logo =
      'https://pbs.twimg.com/profile_images/1042522926971383808/z9w_YpHG_normal.jpg';
    this.keywords = require('../enums/artifact_keywords.json');
    this.apiURL = 'https://playartifact.com/cardset/';
    this.storareDecksURL =
      'https://storage.googleapis.com/roshan-bot.appspot.com/decks%2F';
    this.setsIDs = ['00', '01'];
    ...
    return Promise.all(
      this.setsIDs.map((id) =>
        axios
          .get(this.apiURL + id)
          .then(({ data }) =>
            axios.get((data.cdn_root + data.url).replace('\\', ''))
          )
      )
    ).then((responses) => {
      responses.forEach((response) => {
        ...
        response.card_set.card_list
          .filter((card) => card.card_type === 'Hero')
          .forEach((card) => {
            const refSignatureCard = card.references.find(
              (r) => r.ref_type === 'includes'
            );
            if (refSignatureCard) {
              const signatureCard = response.card_set.card_list.find(
                (c) => c.card_id === refSignatureCard.card_id
              );
              if (signatureCard) {
                signatureCard.references.push({
        ...
        cardsresponse.forEach((cardresponse) => {
          if (!areCards.includes(cardresponse.card_type)) {
            return;
          }
      ...
      this.cards = this.cards.map((card) => {
        if (card.signatureCardFor) {
          ...
          if (heroCard) {
            card.aliasExtended = [heroCard.name.english, ...heroCard.alias].map(
              (name) => name + this.suffixSig
            );
          }
        }
        return card;
      ...
      this.client.logger.info(
        'Artifact Component: ' + `total cards ${this.cards.length}`
      );
  ...
  static getCardColor(data) {
    if (data.is_blue) {
      return 'Blue';
    } else if (data.is_green) {
      return 'Green';
    } else if (data.is_red) {
      return 'Red';
    } else if (data.is_black) {
      return 'Black';
    } else if (data.card_type === 'Item') {
      return 'Yellow';
    } else {
      return '';
    }
  ...
  static getReferenceAbilities(data, dataset) {
    ...
    const abilities = Artifact.parseReferences(data, [
      'active_ability',
      'passive_ability'
    ]);
  ...
  static getReferenceSignatureCard(data, dataset) {
    ...
    if (!parseSignatureCard.length) {
      return;
    }
    const signatureCard = Artifact.getCardByID(
      parseSignatureCard[0].card_id,
      dataset
    );
    return signatureCard.card_name;
  }
  static getReferenceSignatureCardFor(data, dataset) {
    const parseSignatureCardFor = Artifact.parseReferences(data, [
      'references'
    ]);
    if (!parseSignatureCardFor.length) {
      return;
    }
  ...
  static getAliasExtended(data, dataset) {
    const aliasExtended = [];
    const signatureCardFor = Artifact.getReferenceSignatureCardFor(
      data,
      dataset
    );
    if (signatureCardFor) {
      aliasExtended.push(signatureCardFor.english);
    }
  ...
  static getAlias(data, dataset, modifiers) {
    ...
    const alias = [];
    if (
      modifiers[data.card_name.english] &&
      modifiers[data.card_name.english].alias
    ) {
  ...
  static clearHTMLTags(text) {
    return text
      .replace(
        new RegExp(`<span style=\'font-weight:bold;color:#ffffff;\'>`, 'g'),
        ''
      )
      .replace(
        new RegExp(`<span style='font-weight:bold;color:#c2352d;'>`, 'g'),
        ''
      )
      .replace(
        new RegExp(`<span style='font-weight:bold;color:#479036;'>`, 'g'),
        ''
      )
      .replace(
        new RegExp(`<span style='font-weight:bold;color:#2f7492;'>`, 'g'),
        ''
      )
      .replace(
        new RegExp(`<span style='font-weight:bold;color:#736e80;'>`, 'g'),
        ''
      )
  ...
  static createCard(data, dataset) {
    return {
      ...
      signatureCard: Artifact.getReferenceSignatureCard(
        data,
        dataset.card_list
      ),
      signatureCardFor: Artifact.getReferenceSignatureCardFor(
        data,
        dataset.card_list
      ),
      crossLane: false,
  ...
  getCard(name) {
    return this.cards.find((card) =>
      [
        card.name.english.toLowerCase(),
        ...card.alias.map((alias) => alias.toLowerCase()),
        ...card.aliasExtended.map((alias) => alias.toLowerCase())
      ].includes(name.toLowerCase())
    );
  }
  getSet(name) {
    return this.sets.find(
      (set) => set.name.english.toLowerCase() === name.toLowerCase()
    );
  }
  searchCard(text) {
    ...
    return this.cards
      .filter((c) => {
        if (
          [
            c.name.english,
            c.text.english ? c.text.english : '',
            c.signatureCard ? c.signatureCard.english : '',
            c.signatureCardFor ? c.signatureCardFor.english : ''
          ]
            .map((s) => s.toLowerCase())
        ...
        if (
          c.abilities.length &&
          c.abilities.find((s) => s.text.english.includes(text))
        ) {
          return true;
        }
      })
      ...
      embed: {
        ...
        author: {
          name: `${card.name.english}${
            ['Hero', 'Creep'].includes(card.cardType)
              ? ' - ' + card.attack + '/' + card.armor + '/' + card.health
              : ''
          }`,
        ...
        description: `${
          card.abilities.length
            ? card.abilities
                .map(
                  (s) =>
                    `${s.name.english ? '**' + s.name.english + '** ' : ''}(${
                      s.type
                    }${s.type === 'Active' ? ' CD: ' + s.cooldown : ''}) - *${
                      s.text.english
                    }*`
                )
                .join('\n') + '\n'
          ...
          card.signatureCard && card.cardType === 'Hero'
            ? '__**Signature Card:**__ ' + card.signatureCard.english + '\n'
            : ''
        }${
          card.signatureCardFor && card.cardType !== 1
            ? '__**Signature Card for:**__ ' +
              card.signatureCardFor.english +
              '\n'
            : ''
        }${Artifact.iconsCardString(card, replacer)}${
          [...card.alias, ...card.aliasExtended].length
            ? '\n**Alias:** ' +
              [...card.alias, ...card.aliasExtended].map((a) => a).join(', ') +
              '\n'
            : ''
        }${
          data
            ? `\n[:moneybag:](https://steamcommunity.com/market/listings/${this.appID}/1${card.id}) ${data.body.lowest_price}`
            : ``
        }`, //.filter(a => !a.includes(_sig))
        thumbnail: { url: card.imageLarge.default, height: 40, width: 40 },
        footer: {
          icon_url: Artifact.getCardRarityUrl(card),
          text: `Set: ${card.setName.english} - ${card.rarity}`
        },
  ...
  generateDeck(code, alias) {
    try {
      ...
      if (!decoded) {
        throw new Error(`Error in decoding deck: ${code}`);
      }
      if (alias && decoded.name !== alias) {
        ...
        if (!code) {
          throw new Error(`Error in new code: ${oldcode}`);
        }
      ...
      const cardsDeck = [
        ...decoded.heroes.sort((a, b) => a.turn - b.turn),
        ...decoded.cards,
        ...decoded.heroes.map((card) => ({
          id: this.cards.find(
            (signatureCard) =>
              signatureCard.name.english ===
              this.getCardByID(card.id).signatureCard.english
          ).id,
          count: 3
        }))
      ].map((card) => ({
        ...
        turn: card.turn || 0,
        count: card.count || 1
      })); // Todo ordenar por coste y colores
      ...
      const sorting_info = [
        ...
        },
        {
          ...
          sort: (el1, el2) => {
            ...
            if (manaCostDiff === 0) {
              if (el1.card.name.english > el2.card.name.english) {
                return 1;
              }
              return -1;
            }
      ...
      const orderedDeck = sorting_info.reduce((sum, sortMethod) => {
        return [
          ...sum,
          ...cardsDeck.filter(sortMethod.filter).sort(sortMethod.sort)
        ];
      }, []);
      const jimps = [
        new jimp(
          this.configDeckDecoder.width,
          cardsDeck.length * this.configDeckDecoder.rowHeight
        ),
        Promise.all([
        ...
        Promise.all(
          Object.keys(Artifact.hexColors).map(
            (bg) =>
              new jimp(
                this.configDeckDecoder.width,
                this.configDeckDecoder.rowHeight,
                Artifact.hexColors[bg]
              )
          )
        ),
        Promise.all(
          orderedDeck.map((el) => {
            return Promise.all([
              ...
              el.card.signatureCardFor
                ? jimp.read(
                    this.getCardByName(el.card.signatureCardFor.english)
                      .imageIngame.default
                  )
                : null
            ]);
      ...
      return Promise.all(jimps).then((data) => {
        ...
        data[3].forEach((el, index) => {
          const ref = canvas.ref('row' + index, {
            x: 0,
            y: index * this.configDeckDecoder.rowHeight,
            w: this.configDeckDecoder.width,
            h: this.configDeckDecoder.rowHeight
          });
          const cardBg = canvas
            .paint(
              `bg${index}`,
              data[2][Object.keys(Artifact.hexColors).indexOf(el[8])]
            )
            .set(ref, 'cy');
          const imageMini = canvas
            .paint(
              `icon${index}`,
              el[0].resize(this.configDeckDecoder.rowHeight, jimp.AUTO)
            )
            .place(ref, 'cy')
            .set(ref, 'lx', { x: 0 }); //.set(ref, 'lx', { x: this.configDeckDecoder.space4})
          // const costCard = canvas.write('costCard',el[5],'w16').place(imageMini,'rx',{x : 10}).set(ref,'cy')
          const imageTypeCard = canvas
            .paint(
              `cardtype${index}`,
              el[5].resize(this.configDeckDecoder.imageTypeCardSize, jimp.AUTO)
            )
            .set(imageMini, 'rxcy', { x: this.configDeckDecoder.space4 });
          const cardName = canvas
            .write('cardName', el[1], 'w16')
            .place(imageTypeCard, 'rx', { x: this.configDeckDecoder.space24 })
            .set(ref, 'cy');
          if (index < 5) {
            const heroTurn = canvas
              .write('heroTurn', `R${el[3]}`, 'w16')
              .set(ref, 'gxrcy', { x: this.configDeckDecoder.space4 });
            // const cardSymbol = canvas.paint(`cardSymbol${index}`, el[6].resize(12, jimp.AUTO)).set(heroTurn, 'gxlcy', {x : -16})
          } else if (index > 4) {
            const cardCost = canvas
              .write('cardCost', `${el[7]}`, 'w16')
              .place(imageTypeCard, 'rxcy', {
                x: this.configDeckDecoder.space12
              })
              .set(null, 'csx');
            const cardCount = canvas
              .write('cardCount', `x${el[4]}`, 'w16')
              .set(ref, 'gxrcy', { x: this.configDeckDecoder.space4 });
            if (el[9]) {
              const heroSig = canvas
                .paint(
                  `heroSig${index}`,
                  el[9].resize(this.configDeckDecoder.heroSigSize, jimp.AUTO)
                )
                .set(cardCount, 'gxlcy', { x: -24 });
            }
        ...
        return canvas
          ...
          .then((buffer) => ({
            buffer,
            data: { code, name: decoded.name, deckUrl: this.deckCodeUrl(code) }
          }))
          .then((dataDeck) =>
            this.getDeckPrice(dataDeck.data.code).then((price) => {
              dataDeck.data.price = price;
              return dataDeck;
            })
          );
      });
  ...
  static iconsCardString(card, replacer) {
    ...
    return `${
      card.manaCost ? `${replacer.replacer('<mana>')} ${card.manaCost} ` : ''
    } ${
      card.goldCost ? `${replacer.replacer('<gold>')} ${card.goldCost} ` : ''
    } ${card.getInitiative ? '⚡' : ''} ${card.crossLane ? '<>' : ''}`;
  }
  ...
  uploadBufferDeck(buffer, code) {
    return new Promise((res, rej) => {
      const file = this.client.storage.file(
        `${this.pathStorageDecks}/${code}.jpg`
      );
      file
        .createWriteStream({ contentType: 'image/jpeg', public: true })
        .on('error', (err) => {
          rej(err);
        })
        .on('finish', (...val) => {
  ...
  saveDeckIntoCache(code, name, author, url) {
    ...
    return this.client.cache.decks.save(code, {
      name,
      author,
      url,
      ts: new Date()
  ...
  uploadDeckAndCache(buffer, code, name, author) {
    return this.uploadBufferDeck(buffer, code).then((url) =>
      this.saveDeckIntoCache(code, name, author, url)
    );
  }
  uploadDeckAndCacheDiscord(msg, code, name, author, url) {}
  getCachedDeck(search) {
    return this.client.cache.decks.find((deck) =>
      [deck._id, deck.name]
        .map((s) => s.toLowerCase())
  ...
  findAllCachedDeckByName(search) {
    return this.client.cache.decks.filter((deck) =>
      deck.name.toLowerCase().includes(search.toLowerCase())
    );
  }
  getCardPrice(cardname) {
    return new Promise((res, rej) => {
      ...
      if (!card) {
        rej(new Error(`Card doesn't exist: ${cardname}`));
      }
      steamprice.getprice(
        this.appID,
        `1${card.id}`,
    ...
    return new Promise((res, rej) => {
      steamprice.getprice(
        ...
        (err, data) => {
          ...
          !err
            ? res(Number(value.replace(/[-|€]/g, '').replace(',', '.')))
            : rej(err);
        },
        3
      );
  ...
  getDeckPrice(deckcode) {
    return this.getDeckFromCode(deckcode).then((deckData) => {
      const reducedDeck = deckData
        .filter((deckCard) => !deckCard.card.signatureCardFor)
        .map((deckCard) => ({
          card: deckCard.card,
          id: deckCard.card.id,
      ...
      return Promise.all(
        reducedDeck.map((deckCard) => {
          return (
            this.getPriceCard(deckCard.id)
              // .then(data => data || 0)
              .catch((err) => 0)
          );
        })
      ).then((data) => {
        const deckPrice = data.reduce(
          (sum, cur, index) => sum + cur * reducedDeck[index].count,
          0
        );

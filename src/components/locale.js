...
module.exports = class Locale extends Component {
  constructor(client, options) {
    ...
    Eris.Message.prototype.reply = function (
      ...
      if (typeof file === 'string') {
        user_discord_id = file;
        file = undefined;
      }
      return this.channel.createMessage
        ? this.channel.createMessage(
            reply(content, replacements, user_discord_id, this),
            file
          )
        : self.client.createMessage(
            this.channel.id,
            reply(content, replacements, user_discord_id, this),
            file
          );
    };
    Eris.Message.prototype.replyDM = function (
      content,
      replacements,
      file,
      user_discord_id
    ...
    function reply(content, replacements = {}, user_discord_id, msg, fn) {
      ...
      const user =
        (user_discord_id && self.client.users.get(user_discord_id)) ||
        msg.author;
      ...
      return self.replaceContent(
        content,
        replacements,
        msg.author.account.lang
      );
    }
    ...
    this.client.commands.forEach((command) => {
      ...
      if (command.childs.length) {
        command.childs.forEach((subcommand) => {
          localeCommands['cmd_' + command.name + '_' + subcommand.name] =
  ...
  getString(key, language) {
    return this.lang[language] && this.lang[language][key] !== undefined
  ...
  replyInteraction(interaction, content, replacements = {}) {
    ...
    const {
      lang: user_account_lang,
      dota: user_account_dota,
    ...
    const message = this._replaceContent(
      content,

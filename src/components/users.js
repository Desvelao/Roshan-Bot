...
module.exports = class Users extends CustomComponent() {
  ...
  betatesters() {
    const set = new Set([
      ...
      ...Array.from(this.client.cache.betatesters),
      ...this.client.server
        .membersWithRole(this.client.config.roles.betatester)
        .map((m) => m.id)
    ]);
  ...
  supporters() {
    const set = new Set([
      this.client.owner.id,
      ...Array.from(this.client.cache.supporters),
      ...this.client.server
        .membersWithRole(this.client.config.roles.supporter)
        .map((m) => m.id)
    ]);
    return Array.from(set);
  }

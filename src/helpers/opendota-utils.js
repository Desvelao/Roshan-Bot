...
module.exports.winnerTeam = function (table) {
  if (table.radiant_win) {
    if (table.radiant_team) {
      return table.radiant_team.name;
    } else {
      return RADIANT;
    }
  } else {
    if (table.dire_team) {
      return table.dire_team.name;
    } else {
      return DIRE;
    }
  }
...
module.exports.nameAndNick = function (profile) {
  const name = profile.name
    ? profile.personaname + ' (' + profile.name + ')'
    : profile.personaname;
  return parseText(name || unknown, 'nf');
};
...
module.exports.getMedal = function (rank, mode, replace) {
  ...
  let rank_tier = rank.rank_tier,
    leaderboard = rank.leaderboard_rank || null;
  let medal_tier = rank.rank_tier ? parseInt(rank_tier.toString()[0]) : 0,
    medal_range = rank.rank_tier ? parseInt(rank_tier.toString()[1]) : 0;
  let medal,
    medal_raw = { rank: null, leaderboard: null };
  const medals = [
    'norank',
    'herald',
    'guardian',
  ...
  if (rank_tier < 80) {
    if (rank_tier) {
      ...
      medal = '<medal_' + medals[medal_tier] + '>';
    }
  } else {
  ...
  if (mode === 'emoji') {
    ...
    return {
      rank: rank.rank_tier || false,
      leaderboard: rank.leaderboard_rank || false
    };
  }

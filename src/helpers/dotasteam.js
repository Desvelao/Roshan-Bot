...
util.steam = {};
const steamlinks = {
  profiles: 'http://steamcommunity.com/profiles/',
  id: 'http://steamcommunity.com/id/'
};

util.steam.idToUrl = function (id) {
  return (
    (id.match(new RegExp('[^0-9]')) ? steamlinks.id : steamlinks.profiles) + id
  );
};

util.steam.idToLink = function (id, hide, text) {
  return hide
    ? `[${text ? text : id}](${util.steam.idToUrl(id)})`
    : `[${text ? text : id}](${util.steam.idToUrl(id)}) \`${util.steam.idToUrl(
        id
      )}\``;
};
...
util.dota.idToLink = function (id, hide, text, mode) {
  mode = mode || 'dotabuff';
  return hide
    ? `[${text ? text : id}](${util.dota.idToUrl(id, mode)})`
    : `[${text ? text : id}](${util.dota.idToUrl(
        id,
        mode
      )}) \`${util.dota.idToUrl(id, mode)}\``;
};

...
class Element extends Square {
  constructor(name, data, canvas) {
    ...
    this.canvas = canvas;
    if (!this.canvas) {
      console.log('Element NO BASE: ', this.name);
    }
  }
  log() {
...
class Canvas extends Square {
  ...
  write(name, text, font, options) {
    ...
    data.text = text;
    data.font =
      typeof (font === 'string') && this.fonts[font] ? this.fonts[font] : font;
    options = options || {};
  ...
  add(el) {
    if (!(el instanceof Element || el instanceof Image || el instanceof Text)) {
      throw new Error('Elemento no compatible');
    }
...
function startFromNull(value, start) {
...
class Group extends Square {
  constructor(name, elements, base) {
    ...
    this.elements.forEach((el) => {
      if (
        !(el instanceof Element || el instanceof Image || el instanceof Text)
      ) {
        throw new Error('Elemento no compatible para un grupo');
      }
  ...
  place(ref, mode, options) {
    ...
    const dx = mode.includes('x') ? x - before.x : 0,
      dy = mode.includes('y') ? y - before.y : 0;

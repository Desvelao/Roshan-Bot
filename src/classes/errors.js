...
class UserError extends BaseError {
  ...
  toConsole(msg, args, command) {
    const embed = {
      ...
      author: {
        name: `${command.name} - ${msg.author.username} - ${msg.author.id}`,
        icon_url: msg.author.avatarURL
      },
      description: msg._client.components.Locale.replacer(
        this.message,
        this.replacer,
        msg._client.components.Locale.defaultLanguage
      ),
      footer: { text: `Error: ${this.type || 'ND'} - ${command.name || ''}` }
    };
    if (this.err) {
      embed.fields = [
        { name: 'Stack', value: toCode(this.err.stack), inline: false }
      ];
    }
    return { embed };
  }
  ...
  toConsole(msg, args, command) {
    const embed = {
      ...
      fields: [
        {
          name: 'Stack',
          value: toCode(this.err ? this.err.stack : this.stack),
          inline: false
        }
      ],

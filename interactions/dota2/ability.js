...
module.exports = {
    ...
    run: async function (interaction, client, command) {
        ...
        return client.components.Locale.replyInteraction(interaction, {
            embed: {
                author: { name: ability.dname, url: enumAbilities.dotaWikiURL + ability.dname.replace(/ /g, "_") },
                description: ability.desc,
                fields: [
                    ...(attributes ? [{ name: 'Stats', value: attributes, inline: false}] : []),
                    ...(ability.cd ? [{name: 'CD', value: Array.isArray(ability.cd) ? ability.cd.join(', ') : ability.cd, inline: true}] : []),
                    ...(ability.mc ? [{name: '{{{emoji_mana}}}', value: Array.isArray(ability.mc) ? ability.mc.join(', ') : ability.mc, inline: true}] : []),
                ],

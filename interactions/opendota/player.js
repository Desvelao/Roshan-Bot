...
module.exports = {
  ...
  run: async function(interaction, client, command){
    ...
    const top5Heroes = results[2].slice(0,5).reduce((sum, el) => {
      return sum
        + client.components.Locale._replaceContent('top5Heroes', interaction.user.account.lang, { hero: enumHeroes.getValue(el.hero_id).localized_name, wr: odutil.winratio(el.win, el.games - el.win), games: el.games }) + '\n'
    },'')
    ...
    return client.components.Locale.replyInteraction(interaction, {
      embed: {
        ...
        fields: [
          ...
          {name: 'player.top5heroes', value : '{{{top5heroes}}}', inline: false},
        ],
      ...
      wlr: results[1].win + '/' + results[1].lose + ' (' + odutil.winratio(results[1].win, results[1].lose) + '%)',
      kda: results[3][0].sum + '/' + results[3][1].sum + '/' + results[3][2].sum + ' (' + kda + ')',
